...
void addKnownBytes(LocalTreeDiscoveryCounters &counters, quint64 bytes) {
    if (bytes >
        std::numeric_limits<quint64>::max() - counters.knownBytes) {
...
LocalTreeDiscovery::~LocalTreeDiscovery() { stopWorker(); }
...
void LocalTreeDiscovery::setPendingTaskCount(int pendingTaskCount) {
    {
        ...
        if (!backpressurePaused_ &&
            pendingTaskCount_ >= highWatermark_) {
            backpressurePaused_ = true;
        } else if (backpressurePaused_ &&
                   pendingTaskCount_ < lowWatermark_) {
...
bool LocalTreeDiscovery::waitForBackpressure(
...
void LocalTreeDiscovery::start(
    ...
    options.pendingHighWatermark =
        std::max(1, options.pendingHighWatermark);
    options.pendingLowWatermark =
        std::clamp(options.pendingLowWatermark, 0,
                   options.pendingHighWatermark);
        ...
        [this, options = std::move(options),
         generation](std::stop_token stopToken) {
            ...
            auto flush = [&]() {
                ...
                return deliverBatch(std::move(outgoing), generation,
                                    stopToken);
            };
            auto maybeConfirm = [&]() {
            ...
            for (int rootIndex = 0;
                 rootIndex < options.roots.size() &&
                 !cancellationRequested(stopToken);
                 ++rootIndex) {
                const QFileInfo requestedInfo(
                    options.roots[rootIndex].localPath);
                    ...
                    for (const QFileInfo &child : entries) {
                        ...
                        if (child.isDir()) {
                            ...
                            childDirectories.push_back(
                                {rootIndex, child.absoluteFilePath(),
