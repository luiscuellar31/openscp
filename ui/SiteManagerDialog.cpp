...
static bool hasDuplicateSiteName(const QVector<SiteEntry>& sites, const QString& candidate, int ignoreIndex = -1) {
    const QString normalizedCandidate = normalizedSiteName(candidate);
    if (normalizedCandidate.isEmpty()) return false;
    for (int i = 0; i < sites.size(); ++i) {
        if (i == ignoreIndex) continue;
        if (normalizedSiteName(sites[i].name).compare(normalizedCandidate, Qt::CaseInsensitive) == 0) {
            return true;
        }
    }
    return false;
...
static QString legacyNameSecretKey(const QString& siteName, const QString& item) {
    return QString("site:%1:%2").arg(siteName, item);
}

static QString siteSecretKey(const SiteEntry& e, const QString& item) {
    if (!e.id.isEmpty()) return idSecretKey(e.id, item);
    return legacyNameSecretKey(e.name, item);
}

static void removeLegacyNameSecrets(SecretStore& store, const QString& siteName) {
    if (siteName.isEmpty()) return;
    store.removeSecret(legacyNameSecretKey(siteName, QStringLiteral("password")));
    store.removeSecret(legacyNameSecretKey(siteName, QStringLiteral("keypass")));
}
...
void SiteManagerDialog::saveSites() {
    ...
    for (int i = 0; i < sites_.size(); ++i) {
        ...
        // Password and passphrase are stored in SecretStore under keys derived from stable site UUID.
...
void SiteManagerDialog::onEdit() {
    ...
    // Preload site options and stored secrets
    {
        ...
        if (auto pw = store.getSecret(siteSecretKey(e, QStringLiteral("password")))) {
            opt.password = pw->toStdString();
        } else if (auto legacyPw = store.getSecret(legacyNameSecretKey(e.name, QStringLiteral("password")))) {
            opt.password = legacyPw->toStdString();
        }
        if (auto kp = store.getSecret(siteSecretKey(e, QStringLiteral("keypass")))) {
            opt.private_key_passphrase = kp->toStdString();
        } else if (auto legacyKp = store.getSecret(legacyNameSecretKey(e.name, QStringLiteral("keypass")))) {
...
bool SiteManagerDialog::selectedOptions(openscp::SessionOptions& out) const {
    ...
    if (auto pw = store.getSecret(siteSecretKey(selected, QStringLiteral("password")))) {
        out.password = pw->toStdString();
        haveSecret = true;
    } else if (auto legacyPw = store.getSecret(legacyNameSecretKey(name, QStringLiteral("password")))) {
    ...
    if (auto kp = store.getSecret(siteSecretKey(selected, QStringLiteral("keypass")))) {
        ...
        haveSecret = true;
    } else if (auto legacyKp = store.getSecret(legacyNameSecretKey(name, QStringLiteral("keypass")))) {
