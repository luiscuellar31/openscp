...
private:
    ...
    QCheckBox* showQueueOnEnqueue_ = nullptr; // auto-open transfer queue after enqueue
    class QLineEdit* defaultDownloadDirEdit_ = nullptr; // default local download directory
    ...
    class QSpinBox* noHostVerifyTtlMinSpin_ = nullptr; // temporary "no host verification" ttl in minutes
    ...
    class QSpinBox* globalSpeedDefaultSpin_ = nullptr; // default global speed limit KB/s (0 = unlimited)
    ...
    class QSpinBox* stagingPrepTimeoutMsSpin_ = nullptr; // timeout before "wait/cancel" prompt
    class QSpinBox* stagingConfirmItemsSpin_ = nullptr; // threshold for "large batch" confirmation
    class QSpinBox* stagingConfirmMiBSpin_ = nullptr; // threshold in MiB for "large batch" confirmation
