...
void DragAwareTreeView::startDrag(Qt::DropActions supportedActions) {
    ...
    if (indexes.isEmpty()) { QTreeView::startDrag(supportedActions); return; }
    QMimeData* md = model()->mimeData(indexes);
    if (!md) { QTreeView::startDrag(supportedActions); return; }
    auto drag = new QDrag(this);
    drag->setMimeData(md);
    const auto res = drag->exec(Qt::CopyAction);

    // Custom payload from RemoteModel when dragging out from remote
    const QByteArray k = QByteArrayLiteral("application/x-openscp-staging-batch");
    if (!md->hasFormat(k)) return; // nothing to do
    const QString batchDir = QString::fromUtf8(md->data(k));
    if (batchDir.isEmpty()) return;
...
void DragAwareTreeView::showKeepMessage(const QString& batchDir) {
    ...
    QTimer::singleShot(10000, lbl, [mw, lbl]{ mw->statusBar()->removeWidget(lbl); lbl->deleteLater(); });
}

void DragAwareTreeView::showKeepMessageWithPrefix(const QString& prefix, const QString& batchDir) {
    auto* mw = qobject_cast<QMainWindow*>(window());
    if (!mw) return;
    auto* lbl = new QLabel(mw);
    lbl->setTextFormat(Qt::RichText);
    const QString url = QUrl::fromLocalFile(batchDir).toString();
    const QString txt = QString("%1 <a href=\"%2\">%3</a>").arg(prefix, url, batchDir);
    lbl->setText(txt);
    lbl->setOpenExternalLinks(true);
    lbl->setCursor(Qt::PointingHandCursor);
    mw->statusBar()->addPermanentWidget(lbl, 0);
    QTimer::singleShot(12000, lbl, [mw, lbl]{ mw->statusBar()->removeWidget(lbl); lbl->deleteLater(); });
}
...
void DragAwareTreeView::scheduleAutoCleanup(const QString& batchDir) {
    auto tryDelete = [this](const QString& dir) -> bool {
        ...
        QDir rt(root);
        if (rt.exists() && rt.entryList(QDir::NoDotAndDotDot|QDir::AllEntries).isEmpty()) {
            rt.rmdir(".");
        }
        return true;
    };
    ...
    QTimer::singleShot(500, this, [this, batchDir, tryDelete](){
        if (tryDelete(batchDir)) return;
        // Retry up to 3 times at 1s intervals
        auto retries = new int(3);
        auto timer = new QTimer(this);
        timer->setInterval(1000);
        connect(timer, &QTimer::timeout, this, [this, timer, retries, batchDir, tryDelete]() {
            if (tryDelete(batchDir) || --(*retries) <= 0) {
                timer->stop(); timer->deleteLater(); delete retries;
            }
        });
        timer->start();
    });
...
void DragAwareTreeView::showPrepOverlay(const QString& text) {
    if (!overlay_) {
        ...
        overlay_->setStyleSheet("background: rgba(0,0,0,0.35); border: 1px solid rgba(255,255,255,0.25);");
        ...
        QObject::connect(overlayEsc_, &QShortcut::activated, this, [this]{
            cancelCurrentBatch(QStringLiteral("esc"));
        });
        // Simple geometry without layouts to avoid layout warnings inside viewport
    }
    ...
    if (!quitConn_) {
        quitConn_ = QObject::connect(qApp, &QCoreApplication::aboutToQuit, this, [this]{ cancelCurrentBatch(QStringLiteral("quit")); });
    }
}
...
void DragAwareTreeView::updateOverlayGeometry() {
    ...
    overlay_->setGeometry(r.adjusted(r.width()/6, r.height()/3, -r.width()/6, -r.height()/3));
...
void DragAwareTreeView::startRemoteDragAsync(RemoteModel* rm) {
    if (!rm || !transferMgr_) { QTreeView::startDrag(Qt::CopyAction); return; }
    ...
    // Build unique local path helper (for top-level files; folders preserve structure)
    // Normalize to NFC so names are stable across APFS/HFS+ normalization differences
    // (macOS filesystems may store NFD; we normalize for consistent collision checks).
    ...
    auto splitNameMultiExt = [](const QString& fname) {
        ...
        if (firstDot <= 0) return qMakePair(fname, QString());
        return qMakePair(fname.left(firstDot), fname.mid(firstDot)); // base, extSuffix including dot(s)
    };
    auto uniquePath = [&](const QString& dir, const QString& name) {
        ...
        while (QFileInfo::exists(candidate)) {
            candidate = QDir(dir).filePath(QString("%1 (%2)%3").arg(baseName).arg(i++).arg(extSuffix));
        }
        return candidate;
    };
    ...
    for (const QModelIndex& idx : rows) {
        if (enumCancelFlag_ && enumCancelFlag_->load(std::memory_order_relaxed)) { dragInProgress_ = false; return; }
        ...
        if (rm->isDir(idx)) {
            ...
            if (opt.maxDepth < 1) opt.maxDepth = 32;
            opt.skipSymlinks = true; // per requirements: skip symlinks by default
            ...
            rm->enumerateFilesUnderEx(rpath, files, opt, &partial, &someUnknown, &dirCount, &syms, &denied, &unkCountPart);
            if (someUnknown) anySizeUnknown = true;
            totalDirs += dirCount;
            ...
            for (const auto& f : files) {
                if (enumCancelFlag_ && enumCancelFlag_->load(std::memory_order_relaxed)) { dragInProgress_ = false; return; }
                const QString local = QDir(QDir(staging).filePath(nfc(name))).filePath(nfc(f.relativePath));
                QDir().mkpath(QFileInfo(local).dir().absolutePath());
                targets.push_back({ f.remotePath, local, f.hasSize ? f.size : 0 });
                if (f.hasSize) totalBytes += f.size; // unknown sizes not counted
            }
            ...
            quint64 sz = 0; bool hsz = false;
            if (rm->hasSize(idx)) { hsz = true; sz = rm->sizeAt(idx); }
            if (hsz) totalBytes += sz; else { anySizeUnknown = true; unknownSizeCount += 1; }
            targets.push_back({ rpath, lpath, sz });
            totalItems += 1;
        }
    }
    if (enumCancelFlag_ && enumCancelFlag_->load(std::memory_order_relaxed)) { dragInProgress_ = false; return; }
    if (targets.isEmpty()) { dragInProgress_ = false; return; }
    ...
    QString bytesText = QLocale().formattedDataSize((qint64)totalBytes, 1, QLocale::DataSizeIecFormat);
    if (anySizeUnknown) bytesText = QString("~%1 (%2)").arg(bytesText).arg(QLocale().toString((qulonglong)unknownSizeCount));
    qInfo(ocEnum) << "enum batch" << currentBatchId_
                  << "dirs" << QLocale().toString((qulonglong)totalDirs)
                  << "files" << QLocale().toString((qulonglong)totalItems)
                  << "bytes" << bytesText
                  << "enumMs" << enumMs_
                  << "threshold" << ((tooMany||tooBig)?"yes":"no")
                  << "symlinkSkipped" << QLocale().toString((qulonglong)enumSymlinksSkipped_)
    ...
    // Per-batch reservation to avoid intra-batch races. O(1) QSet with NFC+case-fold keys
    // on macOS/Windows to mimic case-insensitive semantics.
    ...
    auto uniqueFullPath = [&](const QString& full) {
...
#if defined(Q_OS_MAC) || defined(Q_OS_WIN)
        // On macOS (APFS/HFS+) and Windows (NTFS) user-facing semantics are often case-insensitive.
        // Build a lowercase set so that "File.txt" and "file.txt" collide predictably.
        QDir d(dir);
        const auto entries = d.entryList(QDir::NoDotAndDotDot | QDir::AllEntries);
        QSet<QString> lower;
        for (const QString& n : entries) lower.insert(nfc(n).toLower());
        auto buildName = [&](int idx) -> QString {
            if (idx == 0) return nName;
        ...
        while (lower.contains(nfc(name).toLower()) || reserved.contains(key) || QFileInfo::exists(abs)) {
            name = buildName(++i);
    ...
    if (!waitTimer_) waitTimer_ = new QTimer(this);
    ...
    stagingConn_ = QObject::connect(transferMgr_, &TransferManager::tasksChanged, this, [this, self, targets, totalDirs, totalItems]() mutable {
        if (!self) return;
        ...
        for (const auto& p : targets) {
            ...
            for (const auto& t : tasks) {
                if (t.type == TransferTask::Type::Download && t.dst == p.local) {
                    matched = true;
                    if (t.status == TransferTask::Status::Done) done++;
                    else if (t.status == TransferTask::Status::Error || t.status == TransferTask::Status::Canceled) failed++;
                    break;
                }
            }
            if (!matched) {
        ...
        if (overlayProgress_) overlayProgress_->setValue(pct);

        if ((done + failed) >= total) {
            // Finished staging
            if (stagingConn_) { QObject::disconnect(stagingConn_); stagingConn_ = QMetaObject::Connection(); }
            hidePrepOverlay();
            ...
            if (failed > 0) {
                ...
                logBatchResult(currentBatchId_, total, failed,
                               QString("result=partial-fail enumDirs=%1 files=%2 bytes=%3 enumMs=%4 stagingMs=%5 symlinkSkipped=%6 denied=%7")
                ...
                currentBatchDir_.clear(); currentBatchId_.clear(); currentBatchTotal_ = 0;
                return;
            }
            ...
            for (const auto& p : targets) urls << QUrl::fromLocalFile(p.local);
            auto* md = new QMimeData();
            md->setUrls(urls);
            md->setData("application/x-openscp-staging-batch", currentBatchDir_.toUtf8());
            ...
            if (res == Qt::IgnoreAction) {
                ...
                const qint64 stagingMs = stagingTimer_.isValid() ? stagingTimer_.elapsed() : -1;
                logBatchResult(currentBatchId_, total, 0,
                               QString("result=canceled enumDirs=%1 files=%2 enumMs=%3 stagingMs=%4 symlinkSkipped=%5 denied=%6")
                               .arg(QLocale().toString((qulonglong)totalDirs))
                               .arg(QLocale().toString((qulonglong)totalItems))
                               .arg(enumMs_)
                               .arg(stagingMs)
                ...
                dragInProgress_ = false;
                currentBatchDir_.clear(); currentBatchId_.clear(); currentBatchTotal_ = 0;
                if (quitConn_) { QObject::disconnect(quitConn_); quitConn_ = QMetaObject::Connection(); }
                return;
            }
            if (!autoClean) {
                showKeepMessage(currentBatchDir_);
                const qint64 stagingMs = stagingTimer_.isValid() ? stagingTimer_.elapsed() : -1;
                logBatchResult(currentBatchId_, total, 0,
                               QString("result=accepted enumDirs=%1 files=%2 enumMs=%3 stagingMs=%4 symlinkSkipped=%5 denied=%6")
                               .arg(QLocale().toString((qulonglong)totalDirs))
                               .arg(QLocale().toString((qulonglong)totalItems))
                               .arg(enumMs_)
                               .arg(stagingMs)
...
void DragAwareTreeView::cancelCurrentBatch(const QString& reason) {
    if (currentBatchDir_.isEmpty()) { hidePrepOverlay(); dragInProgress_ = false; return; }
    if (stagingConn_) { QObject::disconnect(stagingConn_); stagingConn_ = QMetaObject::Connection(); }
    if (waitTimer_) waitTimer_->stop();
    ...
    showKeepMessage(currentBatchDir_);
    // Ensure final outcome includes symlinkSkipped/denied counters even on manual cancel
    const qint64 stagingMs = stagingTimer_.isValid() ? stagingTimer_.elapsed() : -1;
    logBatchResult(currentBatchId_, currentBatchTotal_, 0,
                   QString("result=canceled enumMs=%1 stagingMs=%2 symlinkSkipped=%3 denied=%4 (%5)")
                       .arg(enumMs_)
                       .arg(stagingMs)
                       .arg(QLocale().toString((qulonglong)enumSymlinksSkipped_))
                       .arg(QLocale().toString((qulonglong)enumDenied_))
...
void DragAwareTreeView::logBatchResult(const QString& batchId, int totalItems, int failedItems, const QString& result) {
    if (batchLogged_) return;
    batchLogged_ = true;
    qInfo(ocDrag) << "batch" << batchId << "items" << totalItems << "failed" << failedItems << result;
}
