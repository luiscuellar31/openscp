...
void MainWindow::initializeSyncCoordinator() {
    if (syncCoordinator_ || !remoteOps_ || !transferMgr_)
        return;
    syncCoordinator_ = new SyncCoordinator(remoteOps_, transferMgr_, this);

    connect(
        syncCoordinator_, &SyncCoordinator::progressChanged, this,
    ...
    connect(
        syncCoordinator_, &SyncCoordinator::preparationCanceled, this, [this] {
            closeSyncProgress(syncProgress_);
            statusBar()->showMessage(tr("Folder comparison canceled"), 4000);
        });
    ...
    connect(
        syncCoordinator_, &SyncCoordinator::largeTreeConfirmationRequired, this,
    ...
    connect(
        syncCoordinator_, &SyncCoordinator::preparationReady, this,
        [this](const SyncPreparationResult &prepared) {
            ...
            connect(&dialog, &SyncDialog::checksumRequested, &dialog,
                    [this, &dialog, localRoot,
                     remoteRoot](const QStringList &relativePaths) {
                        ...
                        auto *progress = new QProgressDialog(
                            ...
                            0,
                            std::max(1, static_cast<int>(std::min<qsizetype>(
                                            relativePaths.size() * 2,
                                            std::numeric_limits<int>::max()))),
                            &dialog);
                        progress->setWindowTitle(tr("Compare checksums"));
                        ...
                        progress->show();
                        syncCoordinator_->startChecksums(localRoot, remoteRoot,
                                                         relativePaths);
                    });

            connect(syncCoordinator_, &SyncCoordinator::checksumProgressChanged,
                    &dialog,
                                          ...
                                          .arg(QLocale().formattedDataSize(
                                               ...
                                               QLocale().formattedDataSize(
                                                   displayableByteCount(
                                                       totalBytes)))
                                    : QLocale().formattedDataSize(
                                          displayableByteCount(processedBytes));
                        }
                        syncProgress_->setLabelText(
            ...
            connect(
                syncCoordinator_, &SyncCoordinator::checksumReady, &dialog,
                [this, &dialog, &localSnapshot,
                 &remoteSnapshot](const SyncChecksumResult &result) {
                    closeSyncProgress(syncProgress_);
                    for (SyncSnapshotEntry &entry : localSnapshot) {
                        const auto local =
                            result.localChecksums.constFind(entry.relativePath);
                        const auto remote = result.remoteChecksums.constFind(
                            entry.relativePath);
                        if (local == result.localChecksums.cend() ||
                            remote == result.remoteChecksums.cend()) {
                            continue;
                        }
                    ...
                    if (!result.failures.isEmpty()) {
                        UiAlerts::warning(
                            ...
                            tr("Some checksums could not be calculated:\n"
                               "%1")
                                .arg(result.failures.join(QLatin1Char('\n'))));
                    }
                });
            connect(syncCoordinator_, &SyncCoordinator::checksumFailed, &dialog,
            ...
            connect(syncCoordinator_, &SyncCoordinator::checksumCanceled,
                    &dialog, [this, &dialog] {
                        closeSyncProgress(syncProgress_);
                        dialog.setChecksumBusy(false);
            ...
            if (batchId == 0 || taskCount == 0) {
                statusBar()->showMessage(
                    tr("No synchronization operations were selected"), 4000);
                return;
            }
            statusBar()->showMessage(
                tr("Queued %1 synchronization operations").arg(taskCount),
                6000);
